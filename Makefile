...
dev: format get
	...
	@CGO_ENABLED=$(CGO_ENABLED) GOOS=$(TARGETOS) GOARCH=$(TARGETARCH) \
		go build -v -o kbot -ldflags "-X=github.com/slavaslyder/kbot/cmd.appVersion=$(VERSION)-dev"

# Production build
build: format get
	@echo "Building production version..."
	@CGO_ENABLED=$(CGO_ENABLED) GOOS=$(TARGETOS) GOARCH=$(TARGETARCH) \
		go build -v -o kbot -ldflags "-X=github.com/slavaslyder/kbot/cmd.appVersion=$(VERSION) -w -s"
